	...
	if (canKick)
	{
		...
		if (entity != nullptr)
		{
			if (entity->has_component<InventoryComponent>())
			{
				...
				if (entity->component<InventoryComponent>()->bombKick && entity->component<DirectionComponent>()->direction == direction) //Wenn das Item aufgenommen wurde und die Richtung des Players, mit der Position der Bombe vom Spieler aus �bereinstimmt.
				{
